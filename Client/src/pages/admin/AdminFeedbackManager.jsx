...
export default function AdminFeedbackManager() {
  ...
  useEffect(() => {
    const fetchFeedbacks = async () => {
      ...
      try {
        ...
        // Filter data locally
        const filtered = data.result.content.filter(
          (f) =>
            !search ||
  ...
  useEffect(() => {
    if (feedbacks.content) {
      const filtered = feedbacks.content.filter(
        (f) =>
          !search ||
          f.email.toLowerCase().includes(search.toLowerCase()) ||
          f.content.toLowerCase().includes(search.toLowerCase())
        ...
        <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
          <div>
            <p className="text-sm text-gray-700">
              ...
              trong{" "}
              <span className="font-medium">{feedbacks.totalElements}</span> kết
              quả
            </p>
          </div>
